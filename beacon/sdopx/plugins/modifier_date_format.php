...
if (!function_exists('sdopx_make_timestamp')) {

    function sdopx_make_timestamp($string)
    {
        if (empty($string)) {
            return time();
...
function modifier_date_format($string, $format = null, $default_date = '', $formatter = 'auto')
{
    ...
    if ($string != '' && $string != '0000-00-00' && $string != '0000-00-00 00:00:00') {
        $timestamp = sdopx_make_timestamp($string);
    } elseif ($default_date != '') {
        $timestamp = sdopx_make_timestamp($default_date);
    } else {
        return;
    }
